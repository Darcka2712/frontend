...
import ModulosKPIs from '@/components/administracion/modulos/ModulosKPIs';
import ModulosTable from '@/components/administracion/modulos/ModulosTable';
import ModulosFormModal from '@/components/administracion/modulos/ModulosFormModal';
import ModulosDeleteModal from '@/components/administracion/modulos/ModulosDeleteModal';
...
export default function ModulosPage() {
  ...
  const { data: modulos, loading, hasFiltersApplied, pagination, filters, ui } = state;
  ...
  const loadData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await getModulos(queryParams);
  ...
  const handleSaved = async (formData) => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      if (selected) {
        await updateModulo(selected.id_modulo, formData);
  ...
  const handleDeleted = async () => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });
      await deleteModulo(selected.id_modulo);
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-12 animate-fadeIn pb-20 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-gradient-to-r from-slate-900/40 to-transparent p-8 rounded-[3rem] border border-white/5 shadow-2xl backdrop-blur-sm">
        <div className="space-y-2">
          ...
          <h1 className="text-5xl font-black text-white tracking-tight">
            Gestión de <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">Módulos</span>
          </h1>
          <p className="text-slate-400 text-lg max-w-2xl font-medium">
            Administra los módulos funcionales y sus capacidades dentro del ecosistema corporativo.
          </p>
        </div>
      ...
      {/* KPIs Section */}
      <ModulosKPIs modulos={modulos} pagination={pagination} />
      ...
      <div className="bg-gradient-to-br from-slate-900/80 via-slate-800/60 to-slate-900/80 backdrop-blur-xl border border-slate-700/30 rounded-[2.5rem] p-8 shadow-2xl shadow-black/20">
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Busca módulos por clave o nombre</p>
          </div>
        </div>
