...
import { Server, CheckCircle, XCircle, LayoutGrid } from 'lucide-react';
import { motion } from 'framer-motion';

const SistemasKPIs = ({ sistemas, pagination }) => {
  const total = pagination?.total || sistemas.length;
  const activos = sistemas.filter(s => s.activo).length;
  const inactivos = total - activos;

  const stats = [
    { label: 'Total Sistemas', value: total, color: 'text-indigo-400', icon: Server, bg: 'bg-indigo-500/10' },
    { label: 'En Línea', value: activos, color: 'text-emerald-400', icon: CheckCircle, bg: 'bg-emerald-500/10' },
    { label: 'Offline', value: inactivos, color: 'text-red-400', icon: XCircle, bg: 'bg-red-500/10' },
