...
import { Apple, CheckCircle, XCircle, TrendingUp } from 'lucide-react';

const FrutaKPIs = ({ frutas = [] }) => {
  const total = frutas.length;
  const activos = frutas.filter(f => f.activo).length;
  const inactivos = total - activos;
  const stats = [
    { label: 'Total Frutas', value: total, icon: Apple, color: 'from-red-600 to-pink-700', shadow: 'rgba(239,68,68,0.3)' },
