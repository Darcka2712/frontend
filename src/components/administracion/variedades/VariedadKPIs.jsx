...
import { Flower, CheckCircle, XCircle, TrendingUp } from 'lucide-react';

const VariedadKPIs = ({ variedades = [] }) => {
  const total = variedades.length;
  const activos = variedades.filter(v => v.activo).length;
  const inactivos = total - activos;
  const stats = [
    { label: 'Total Variedades', value: total, icon: Flower, color: 'from-purple-600 to-pink-700', shadow: 'rgba(147,51,234,0.3)' },
    { label: 'Activas', value: activos, icon: CheckCircle, color: 'from-emerald-600 to-green-700', shadow: 'rgba(16,185,129,0.3)' },
