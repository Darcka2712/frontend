...
import { TreePine, CheckCircle, XCircle, TrendingUp } from 'lucide-react';

const RanchoKPIs = ({ ranchos = [] }) => {
  const total = ranchos.length;
  const activos = ranchos.filter(r => r.activo).length;
  const inactivos = total - activos;

  const stats = [
    { label: 'Total Ranchos', value: total, icon: TreePine, color: 'from-green-600 to-emerald-700', shadow: 'rgba(34,197,94,0.3)' },
