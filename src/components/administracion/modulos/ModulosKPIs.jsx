...
import { Layers, CheckCircle, XCircle, LayoutGrid } from 'lucide-react';
import { motion } from 'framer-motion';

const ModulosKPIs = ({ modulos, pagination }) => {
  const total = pagination?.total || modulos.length;
  const activos = modulos.filter(m => m.activo).length;
  const inactivos = total - activos;

  const stats = [
    { label: 'Total Módulos', value: total, color: 'text-indigo-400', icon: Layers, bg: 'bg-indigo-500/10' },
