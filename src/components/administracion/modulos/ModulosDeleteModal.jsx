...
export default function ModulosDeleteModal({ open, onClose, onConfirm, selected, loading }) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      >
        <div className="flex flex-col items-center p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Desactivar Módulo</h3>
          <p className="text-sm text-slate-400 mb-6">
            ¿Estás seguro de que deseas desactivar <span className="font-bold text-white">{selected.nombre}</span>?
            <br />
            <span className="text-xs text-slate-500">Esta acción lo marcará como inactivo y no se mostrará en el sistema.</span>
          </p>
          <div className="w-full rounded-xl bg-slate-950 border border-slate-800 p-4 mb-6">
            <div className="flex justify-between text-xs text-slate-400 mb-1">
              <span>Clave:</span>
              <span className="font-mono text-white">{selected.clave}</span>
            </div>
            <div className="flex justify-between text-xs text-slate-400">
              <span>Sistema:</span>
              <span className="text-white">{selected.sistema?.nombre || 'N/A'}</span>
            </div>
          </div>
        </div>
