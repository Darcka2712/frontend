...
export default function ModulosFormModal({ open, onClose, selected, onSaved, sistemas, loading }) {
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.id_sistema) newErrors.id_sistema = 'El sistema es requerido';
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-800 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-slate-100">{selected ? 'Editar Módulo' : 'Nuevo Módulo'}</h3>
              <p className="text-xs text-slate-400">{selected ? 'Modificar propiedades del módulo' : 'Registrar un nuevo módulo'}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <SelectField
            label="Ecosistema Maestro"
            options={sistemas.map(s => ({ value: s.id_sistema, label: s.nombre }))}
            value={formData.id_sistema}
            onChange={(val) => setFormData(prev => ({ ...prev, id_sistema: val }))}
            error={errors.id_sistema}
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-800 bg-slate-950/30 p-6">
          ...
          >
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : (selected ? 'Guardar Cambios' : 'Crear Módulo')}
          </button>
        </div>
      </motion.div>
    </div>,
