...
export default function CultivosTiposFormModal({ open, onClose, selected, onSaved, loading }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) return;
    onSaved({ nombre: formData.nombre, activo: formData.activo });
  };
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-800 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-slate-100">{selected ? 'Editar Tipo' : 'Nuevo Tipo de Cultivo'}</h3>
              <p className="text-xs text-slate-400">{selected ? 'Modificar especie de cultivo' : 'Registrar nueva especie'}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          >
            <div>
              ...
              <p className="text-[10px] text-slate-500 uppercase">{formData.activo ? 'Activo en catálogo' : 'En mantenimiento'}</p>
            </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-800 bg-slate-950/30 p-6">
          ...
          >
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : (selected ? 'Guardar Cambios' : 'Registrar Tipo')}
          </button>
        </div>
      </motion.div>
    </div>,
