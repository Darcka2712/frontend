...
export default function PermisosDeleteModal({ open, onClose, onConfirm, selected, loading }) {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      >
        <div className="flex flex-col items-center p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Desactivar Permiso</h3>
          <p className="text-sm text-slate-400 mb-6">
            ¿Estás seguro de que deseas desactivar el permiso <span className="font-bold text-white">{selected.accion}</span>?
            <br />
            <span className="text-xs text-slate-500">Los roles asociados perderán este acceso.</span>
          </p>
          <div className="w-full rounded-xl bg-slate-950 border border-slate-800 p-4 mb-6 space-y-2">
            <div className="flex justify-between text-xs text-slate-400">
              <span>Módulo:</span>
              <span className="text-white">{selected.modulo?.nombre || 'N/A'}</span>
            </div>
            <div className="flex justify-between text-xs text-slate-400">
              <span>Acción:</span>
              <span className="font-mono text-white uppercase">{selected.accion}</span>
            </div>
          </div>
        </div>
