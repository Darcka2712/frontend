...
import { Shield, CheckCircle, XCircle, LayoutGrid } from 'lucide-react';
import { motion } from 'framer-motion';

const PermisosKPIs = ({ permisos, pagination }) => {
  const total = pagination?.total || permisos.length;
  const activos = permisos.filter(p => p.activo).length;
  const inactivos = total - activos;

  const stats = [
    { label: 'Total Permisos', value: total, color: 'text-emerald-400', icon: Shield, bg: 'bg-emerald-500/10' },
