...
import { Sprout, CheckCircle, XCircle, TrendingUp } from 'lucide-react';

const CultivoKPIs = ({ cultivos = [] }) => {
  const total = cultivos.length;
  const activos = cultivos.filter(c => c.activo).length;
  const inactivos = total - activos;
  const stats = [
    { label: 'Total Cultivos', value: total, icon: Sprout, color: 'from-green-600 to-emerald-700', shadow: 'rgba(34,197,94,0.3)' },
    { label: 'Activos', value: activos, icon: CheckCircle, color: 'from-emerald-600 to-green-700', shadow: 'rgba(16,185,129,0.3)' },
    { label: 'Inactivos', value: inactivos, icon: XCircle, color: 'from-red-600 to-rose-700', shadow: 'rgba(239,68,68,0.3)' },
