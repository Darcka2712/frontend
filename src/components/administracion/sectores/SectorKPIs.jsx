...
import { Map, CheckCircle, XCircle, TrendingUp } from 'lucide-react';

const SectorKPIs = ({ sectores = [] }) => {
  const total = sectores.length;
  const activos = sectores.filter(s => s.activo).length;
  const inactivos = total - activos;
  const stats = [
    { label: 'Total Sectores', value: total, icon: Map, color: 'from-orange-600 to-amber-700', shadow: 'rgba(251,146,60,0.3)' },
